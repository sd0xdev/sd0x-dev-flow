...
| S1 | 序列化後不得含 hook-parsed sentinel 字串（gate 標頭行、bare Ready / Mergeable / Blocked / All-Pass 記號）——以名稱描述 gate |
| SCHEMA | 結構完整性：`intent`/`done_definition` 非空、`steps` 為陣列、`kind` 已知、step `id` 必填且唯一、`depends_on` 為陣列且引用存在的 id |
