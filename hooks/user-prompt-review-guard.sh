...
if [[ "$_ADV_OK" == "true" ]]; then
  # Derived reads already re-read the tree — and see through ignore=all,
  # unreadable dirs, and hidden submodules the porcelain probe cannot — so a
  # second git-status downgrade could only undo that (fail-open, R4-1 class).
  # __ADV_DERIVED__ ≠ __GIT_UNAVAILABLE__ keeps the degraded token honest.
