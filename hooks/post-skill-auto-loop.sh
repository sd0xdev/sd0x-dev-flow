#!/usr/bin/env bash
# PostToolUse(Skill) hook: one static gate-sequence reminder. The zero-read
# design: the hook protocol does expose the skill name and result on stdin,
# but this hook DELIBERATELY does not inspect them — the wording is
# unconditional, the model knows where it is. Markdown out, exit 0 always.
...
cat >/dev/null 2>&1 || true  # drain stdin WITHOUT inspecting it — the zero-read design

printf '%s\n' "review → precommit → doc-sync 的閘門順序見 rules/auto-loop.md；本行不知道你剛跑了哪個 skill"
exit 0
