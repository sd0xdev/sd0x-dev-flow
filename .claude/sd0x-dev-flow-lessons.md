...
## L9 — 對「檔案裡有什麼」下斷言，卻沒有去讀那個檔案

- **Context**: create-pr-stacked r2 的文件輪次，同一型錯誤被 Codex 抓到四次。(1) 我用「改檔名會牽動一整批 inbound link」當作不切分 2243 行 review log 的 `[DEVIATION]` 理由——`grep -rn` 出來**只有一個** inbound markdown 連結，那條 DEVIATION 的前提是假的，切分因此照做。(2) 寫「十四個相對連結」——實測 **17**。(3) 寫「第 27–37 輪在 request 的 `## Review Log` 本節以下」——那節總共只有 5 行，輪次其實在正文。(4) 列了一張輪次→Note 條目的對應表——第 27 輪在 AC-Q1 正文、28–30 併進它的續段、32–33 是我漏掉的合併 Note、第 34 輪根本沒有具名條目
- **Error pattern**: 四次的句子都「讀起來很合理」，因為它們是從我對檔案的**印象**生成的，不是從檔案生成的。危險的是這型錯誤沒有失敗訊號：估錯的數字和量出來的數字在句子裡長得一模一樣，而且愈是拿它當論證前提（案例 1 是拿去擋一條規則），愈不會回頭去查。案例 2 更明確——我正在**修正**一個定位錯誤，修正句本身又是沒查證的定位
- **Correct approach**: 凡是句子裡出現**數量**（幾個連結、幾行、幾處引用）或**位置**（在哪一節、在哪個檔、對應哪個條目），先跑指令再寫句子，句子裡的數字直接抄指令輸出。`grep -c` 數數量、`grep -rn` 找 inbound、`wc -l` 量長度、`grep -n` 定位置、`sed -n 'a,bp'` 讀那一段確認它真的是我說的東西。印象只能用來決定「該查哪個檔」，不能用來當句子的內容
- **Prevention**: **沒有 hook 或測試會機械式強制 claim-provenance 配對**——沒有任何自動檢查在辨識「這句話是計數／定位斷言」並回頭找它的出處（`check-comment-blocks.js` 只數註解行數，`fc-doc-currency.js` 只比 git 時間戳）。會抓到的是 `/codex-review-doc`：上面四個案例全部由它擋下，而那是**事後**攔截、每次都要一輪來回。所以這裡給的是寫的當下**要注意的訊號**，用途是讓它不必走到審查那一步。訊號有兩個，都是句子層級、寫的當下就看得到：(1) 句子裡出現**具體數字或位置詞**（幾個、一整批、在哪一節、對應哪個條目）而我**指不出它是哪一次工具輸出來的**——指不出就是猜的，當場補跑 `grep -c` / `grep -rn` / `wc -l` / `grep -n`；(2) 出現「很多／一整批／牽動很廣」這類**量級形容詞被拿來當論證的承重點**（案例 1 就是拿它擋一條規則）——這種句子的整條理由都靠那個量級成立，未量到數字之前不能引用。`[DEVIATION]` 的 `signal=` 欄位要求「a measured value」，正是這條的成文版本；規則已經寫在那裡，缺的是寫句子時把它套用到自己身上
- **Source**: 2026-08-04 — create-pr-stacked r2 文件輪次，切分 review log 期間的四次未量測斷言
