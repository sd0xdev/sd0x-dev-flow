...
> **量測更正（2026-08-21，push-gate-optin doc review round 27 記入）**：上表 D1／D2 的數字經獨立重測，有五處與現況不符。**本文不改寫**——D1／D2 是設計記錄的條目，更正一律追加。**本更正屬 `smart-commit-hardening` 這個變更，不屬 push-gate-optin**；記在這裡只因兩者共用同一棵工作樹，被同一輪 doc review 掃到。
>
> | 位置 | 原文所寫 | 重測值 | 推導 |
> | ---- | -------- | ------ | ---- |
> | D1 的 `create-pr-stacked` 指標 | `create-pr-stacked/2-tech-spec.md:178` | `create-pr-stacked/2-tech-spec/1-core-logic.md` 的 **item 25** | 該檔已依 `@rules/docs-numbering.md` § Size Limit 拆為 `2-tech-spec/` 目錄，`:178` 當時指的是 item 25，該項落在切出的 `1-core-logic.md`。**此列為 2026-08-22 round 68 補記**：這筆更正一度被**寫進 D1 列本身**，那正是上一段禁止的事——記錄的更正追加於此，原列已按 `HEAD` 逐字還原 |
> | D1 淨增量 | 「540 行（較上一輪紀錄的 527 行淨增 6 行）」 | 淨增 **13** 行 | 540 − 527 = 13。540 與 527 兩個數字本身都對，相減寫錯 |
> | D1 §3 區間 | lines 35–433，**399** 行 | lines 35–**446**，**412** 行 | `grep -n '^## '` 得 `## 3.` 於 `:35`、下一個真章節 `## 4.` 於 `:447`，故 §3 迄於 `:446` |
> | D1 §3 佔比 | 73.9% | **76.3%** | 412 ÷ 540。原值是用 399 這個偏低的分子算的 |
> | D1 圍籬內標題行號 | `:422` 的 `## Commit Plan` | `:435` | 同一道 `grep -n` 回報 435；該行確實在 §3.8 的圍籬內，是示範輸出不是節界——這一點原文判斷正確，只有行號漂移 |
> | D2 `4-implementation.md` | 1750 行，§10 起於 `:459`、§11 起於 `:1642` | **1751** 行、`:460` / `:1643`（**2026-08-21 round 27 當刻**；該檔其後又被 round 28 的引用重指改動，round 29 量到 **1753** 行、`:462` / `:1645`——§10 長度 1183 行不變） | `wc -l` → 1751；`grep -n '^## 1[01]'` → 460 與 1643 |
>
> **未受影響的數字**：§10 為 1183 行（460–1642）與 67.6% 的佔比、§10 的 **18** 個 `###` 子節，兩者重跑原指令皆複現。**引用計數則不在此列（2026-08-21 round 28 補）**：指向 `4-implementation.md` 的引用，D2 記為 **32** 行，同一道指令現回報 **34**——多出的兩行正是本註記自己（上表的 D2 列與本句各提及該檔名一次）。這是個自我指涉的度量：任何討論它的更正都會把它推高，所以引用時務必連同量測時刻一起引，或改用把本註記排除在外的指令。D1／D2 的拆檔結論不變——分子偏低反而低估了該拆的程度。
>
> **本註記自己也改變了分母**：上表所有 `2-tech-spec.md` 的數字（540、412、76.3%）都是**追加本註記之前**量到的，與 D1 原文同一個時間點，所以可以逐項對照；追加之後 `wc -l` 為 **554** 行，§3 的區間與 412 行長度不變（本註記落在 §8，不在 §3 內），佔比則變為 74.4%。把行數寫進文件本身就會改變行數——引用時務必連同「哪一刻量的」一起引。
