...
### 3.2 The `--directory` collapse — why the two planes resolve it differently

`git ls-files --directory` collapses a tree at its **first** matching ancestor, emitting one `dir/` node instead of its descendants. That single behaviour creates the same hole on two planes and gets opposite treatments.

**Ignored plane — `ignored_dirs_sha256` (keep the collapse).** The file-level `ignored_content` enumeration lists ignored *files* only: `ls-files -o -i` recurses into ignored dirs and emits leaf files, never a trailing-slash dir entry, and `hashPathsContent`'s ancestor-mode folding only fires for dirs that contain a leaf. So an **empty** ignored dir — created, deleted or chmod'd by an adversarial worker post-snapshot — is invisible to porcelain (`-uall` never lists ignored or empty) *and* to the content digest, and `compare` returned `{ok:true}` despite a real filesystem mutation, breaking the "zero filesystem change" report-only proof (Codex iter-14 P2). `--directory` collapses each ignored dir to a single `dir/` node, empty ones included (verified on git 2.54), so a digest of sorted `path\0mode` drifts on create (new node), delete (node vanishes) and chmod (mode changes). An `lstat` failure **throws** → snapshot's fail-closed catch, symmetric with `hashPathsContent`.

> **Residual (documented, accepted):** because the collapse stops at the first ignored ancestor, a nested **empty** dir created below an already-ignored dir (`cache/emptysub/` when `cache/` is ignored) is never enumerated individually — only `cache/`'s node is. Non-empty descendants are still caught: files by `ignored_content`, their ancestor modes by `hashPathsContent`. Only nested empty dirs escape. This sits behind the read-only admission allowlist (workers cannot `mkdir` through their tools) and is consistent with the `IGNORE_EXCLUDE_PATHSPECS` high-volume-subtree residual.

**Untracked plane — `expandUntrackedDirTree` (expand through the collapse).** Here the same collapse is not a residual but a defeat. A baseline already containing `scratch/file` enumerates exactly `scratch/`, and then `mkdir scratch/empty`, `rmdir` on it, or a `chmod` of any nested dir moves **no** digest on **any** plane — porcelain never lists directories and the file-level untracked scan yields only leaves. Verified against real git: with `scratch/{file.txt,nested/deep/y.txt,emptydir}` untracked, `--directory` emits the single node `scratch/`. That is the common case rather than an exotic one; this repo's own `test/hooks/helpers/` collapses the same way.

The asymmetry is a cost judgement, not an inconsistency. Recursing into `node_modules/` is exactly the blowup `IGNORE_EXCLUDE_PATHSPECS` exists to prevent, so the ignored plane keeps the collapse. On the untracked plane the marginal cost is provably negligible: every *file* in those trees is already enumerated **and** content-hashed by `untracked_content`, so any tree large enough to make the `readdir` walk expensive is already costing far more on a plane that predates it. The walk adds no new order of magnitude.

Symlinks are not followed: `withFileTypes` populates each `Dirent` from the directory entry itself, so a symlink-to-directory reports `isSymbolicLink()` and never `isDirectory()`, and the walk cannot be steered outside the tree by planting one. The caller's three exclusion predicates are re-applied per level, because `--directory` only ever revealed the root of each tree.
