...
每次 session 開頭載入 `CLAUDE.md`(201) + `.claude/CLAUDE.md`(189) + `rules/*.md`(688) ≈ **1,078 行**，其中大量是命令編排與重複強調。
...
| `docs/skill-catalog.yml` | Reference | 遷移後的 catalog 真實來源；需定義 public / internal / alias 分類 |
...
- [ ] `rules/auto-loop.md` 以單一終端完成不變式取代命令編排，且不含「same reply」祈使
- [ ] 保留項全數存活：獨立審查、驗證、round cap、人工升級、安全/資料完整性強制升級
- [ ] `CLAUDE.md` 與 `CLAUDE.template.md` 的必跑 gate 敘述維持一致（皆為 `/precommit`），且 template 的測試指令仍為 `{TEST_COMMAND}` placeholder（未被寫死）
- [ ] 經 `/project-setup` 由 template 渲染出的 `.claude/CLAUDE.md`（非原始 template）解析為本 repo 的 `find` 形式測試指令，`**` glob 寫法零殘留
- [ ] tracked CLAUDE 檔移除 90+ 列指令表；**以導出指令重新產生的清單中，零個測試仍直接對 Quick Reference 表格斷言**，全部遷移至 catalog 契約且測試全綠，且 `claude-md-coverage.test.js:127-257` 的 terminal-gate routing 斷言仍存活
- [ ] 新增迴歸檢查，涵蓋三種寫法而非只有標題字串：(a) 對 `## Command Quick Reference` 區塊斷言、(b) 裸指令 regex（如 `/\/ask/`）、(c) 表格列 regex（如 `` /\|\s*`\/recap-ask`[^\n]+\|/ ``）；並釘住 allowlist 的 3 個非註冊消費者身分，使新增的 CLAUDE 讀取者必須明確分類
- [ ] 表格移除後跑完整測試套件作為行為層後盾（`recap-ask` / `post-dev-recap` 容忍全數缺席，靜態檢查不足以證明遷移完成）
- [ ] comments-only 敘述更正為「code 檔的註解仍保守歸類為 code」，`CLAUDE.md` 與 `hooks/stop-check.md` 同步
- [ ] 常駐 context 總行數較改造前下降 ≥ 40%（以 `wc -l` 實測前後對比為證）
- [ ] 所有 hook 解析的 sentinel 詞彙與 `[NIT_DEFERRED]` 欄位順序**未被更動**（hook 測試全綠）
