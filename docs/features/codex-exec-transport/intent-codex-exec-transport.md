...
- The adapter renders no prompts, parses no sentinels, keeps no state **across dispatches** (its
  per-dispatch `progress.json`/`events.jsonl` live in the alloc dir and die with `cleanup` — qualified
  2026-09-05, work item 7), retries nothing, owns no timeout (the stall advisory kills nothing); no
  tier→profile mapping in v1; no new Anchor Register item, gate sentinel or human exit.
...
- `INV-002`: **Thin, transport-only adapter.** `scripts/codex-exec.js` is ≤290 lines (150 → 220 on
  2026-09-05 for the progress channel — work item 7; 220 → 290 the same day for the alloc-time
  stale sweep, its inode-bound quarantine deletion and its owner-pid liveness check — work item 8; still one file, no abstraction layer), zero npm
  ...
  (`on-failure` has no headless equivalent); exactly one JSONL event (`thread.started`) has
  **verdict** semantics — `item.started`/`item.completed` and `turn.completed` feed the per-dispatch
  progress channel only and decide nothing (work item 7, 2026-09-05).
- `INV-003`: **Exit semantics are the probe — for `start`/`resume`.** Exit 0 ⇔ Codex exited 0 ∧
  `protocol` echoed ∧ valid `threadId` ∧ non-empty report that is still a regular file at exactly
  `0600` ∧ the prompt stream reached `finish`
...
- `INV-007`: **Permission surface grows only where transport is owned.** Direct callers gain
  `Bash(node:*)` + Write + Read; delegating skills gain no **transport** grant — the one
  exception, added by work item 7 (2026-09-05), is the observation grant `Monitor`, which the
  parent-session `codex-code-review` family and its three thin entry points hold so a background
  dispatch can be observed through `progress.json`, and which no `context: fork` skill may hold
  (its notifications could not reach the conversation); the scratch lifecycle is the
