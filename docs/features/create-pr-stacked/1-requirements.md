...
多 Agent、同 repo、功能彼此相依的開發模式下，Agent 產出速度遠快於 code review 消化速度。現行 `/create-pr` 對相依 PR 只有兩個弱支援：Edge Case 表的「Stacked PRs (B → A → main) → body 註記 `Stacked on #N`」（`skills/create-pr/SKILL.md:268`）與 Multi-PR Mode 的依序建立（`skills/create-pr/SKILL.md:257`）。GitHub 已於 2026-07-30 將原生 Stacked Pull Requests 推入 public preview（含 `gh-stack` CLI extension），提供正式的 stack 建模：每層 PR 只顯示該層 diff、底層 merge 後自動 re-target、branch protection 依 stack 最終目標 branch 套用。`/create-pr` 需要一個 stacked 模式，把「建立/更新一條 PR 鏈」變成一等公民操作，而不是手工逐一建 PR 再靠 body 註記表達依賴。
...
| 偵測既有 stack 並支援逐層 update（title/body 刷新） | 自動執行 `gh stack rebase` / `gh stack push` / cascading rebase（見 Constraints — Anchor #4） |
| `gh-stack` 未安裝或 repo 未被 preview rollout 覆蓋時，明確降級至現行行為 | auto-merge / merge queue 整合（官方文件目前互相矛盾，preview 階段不納入） |
...
| `/pr-summary` | Dependent | 現以「base 非 main/master/develop」啟發式偵測 stacked PR（`skills/pr-summary/SKILL.md:44`）；native stack metadata 出現後偵測方式需對齊 |
...
| UC-2 | 開發者 | stack 中某層有新 commit 後要求刷新 | 受影響各層 PR 進入 update mode，逐層重生 title/body 並顯示 before/after diff |
| UC-3 | 開發者 | 在已有 native stack 的 branch 上執行 `/create-pr` | 偵測到 stack，輸出 stack 狀態表（層序、head/base、PR 編號、狀態）而非誤建重複 PR |
| UC-4 | 開發者 | 在未安裝 `gh-stack` 或 repo 未被 rollout 覆蓋的環境執行 stacked 模式 | 明確訊息說明缺什麼 + 降級為現行 Multi-PR Mode；body 依賴標記依模式適配（下層 PR 已存在 → `Stacked on #N`；尚未建立 → 下層 branch 名標記），不留半完成狀態 |
...
| FR-2 | 所有 stack 操作預設 dry-run，輸出可複製貼上的指令；`--execute` 僅沿用現行已授權的 `gh pr create` / `gh pr edit`（`skills/create-pr/SKILL.md` Step 5a 與 Steps 6-7 既有契約，`:165-223`），branch push / rebase / force-push 一律不執行 | Must | Anchor #4 禁止清單是 git mutation（push/rebase 等），不含 `gh pr create/edit`；後者已是現行 execute 模式的一部分（見 Constraints C-1） |
...
| FR-4 | 前置偵測：`gh-stack` extension 是否安裝、repo 是否支援 native stack；缺任一 → 明確訊息 + 降級為現行 Multi-PR 行為 | Must | Public preview 逐步 rollout（2026-07-30 起），環境不齊是常態而非例外；本機目前即未安裝（`gh extension list` 實測） |
...
| FR-7 | update mode 支援 stack：偵測既有 stack 的各層 PR，逐層執行現行 update 流程（smart diff、只更新變動欄位） | Should | stack 生命週期長，刷新是高頻操作 |
...
|----|----------|-------------|--------|
| NFR-1 | Security / Governance | stacked 模式不得執行或誘導執行 Anchor #4 禁止操作（rebase、force-push、push）；需要時輸出指令由使用者自行執行，或指向授權工作流 | skill 測試斷言輸出不含直接執行該類指令的步驟；`allowed-tools` 不擴增 |
...
| NFR-3 | Usability | dry-run 輸出可直接複製貼上執行；降級訊息說明「缺什麼、怎麼補」 | 人工驗證：dry-run 指令原樣貼上可執行 |
| NFR-4 | Maintainability | 依 Development Rule 5：**新增** `test/skills/create-pr.test.js`（目前不存在），覆蓋 stacked 模式 happy path + 降級 + 拒絕路徑；既有 `test/skills/create-pr-sanitization.test.js` 的 sanitization regression 全數保留 | `npm test` 通過；新增案例含 null/空 chain/單層極端值；既有 sanitization 測試無刪減 |
| NFR-5 | Compatibility | 明示環境需求（gh 版本、`gh-stack` extension、repo rollout 狀態）並於執行時偵測，不假設齊備 | 前置偵測步驟存在且缺件時走 FR-4 降級 |
| NFR-6 | Maintainability | SKILL.md 總行數受 `@rules/docs-numbering.md` 500 行上限約束（現為 294 行） | `wc -l skills/create-pr/SKILL.md` ≤ 500；超標則拆 references/ |
...
| Constraint (C-2) | GitHub Stacked PR 為 public preview（2026-07-30 發布），API、CLI 行為、rollout 覆蓋皆可能變動；merge queue 支援分批 rollout 中 | GitHub Changelog（見 § References，web 驗證） |
...
| Assumption (A-2) | 目標 repo 已被 preview rollout 覆蓋 — 未實測，FR-4 的偵測即為此假設的防護 | Inferred |
...
- Signal 2（FR-4）：在 `gh-stack` 未安裝的環境執行，得到明確缺件訊息與降級輸出（現行 Multi-PR + 模式適配的依賴標記：下層 PR 已存在用 `#N`、尚未建立用下層 branch 名），流程不中斷於未定義狀態。
...
- `skills/create-pr/SKILL.md` — 現行流程；`:257` Multi-PR Mode、`:268` Stacked PRs edge case
- `skills/epic-merge/SKILL.md` — 既有 linear stacked PR chain 合併工作流（chain 驗證、backup tags、per-iteration gate）
- `skills/pr-summary/SKILL.md:44` — 現行 stacked PR 偵測啟發式
